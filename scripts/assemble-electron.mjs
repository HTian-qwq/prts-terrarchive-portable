...
export async function assemble(options, { versions = electronVersions } = {}) {
  const paths = {}
  for (const name of ['electronDir', 'dshSource', 'plugin', 'corpusReleases', 'out']) {
  ...
  for (const name of ['electronDir', 'dshSource', 'plugin', 'corpusReleases']) {
  ...
  cpSync(paths.electronDir, paths.out, { recursive: true, dereference: true, filter: () => true })
  ...
  writeFileSync(join(paths.out, '使用说明.txt'), [
    ...
    '本便携版关闭官方应用自动更新；升级时解压新版并保留自己的 userdata/。',
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const { values } = parseArgs({ options: Object.fromEntries(
    ['electron-dir', 'dsh-source', 'plugin', 'corpus-releases', 'out'].map((name) => [name, { type: 'string' }]),
  ) })
  const manifest = await assemble({
    electronDir: values['electron-dir'], dshSource: values['dsh-source'], plugin: values.plugin,
